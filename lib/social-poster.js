...
async function postToFacebook({ message, link }) {
  ...
  // FB's /feed endpoint occasionally sits on the request for 30-45s under
  // load — the post still lands, we just don't get an ID back in time. A
  // 20s ceiling was catching those and marking real successes as failures.
...
function buildPostLines(job) {
  // Sponsored posts (Wasla / Finalizat / Rezolyzer) have no HR email and
  // link to the sponsor's own careers page (or wzyfa's form for those
  // without one). Everything else stays the same visual rhythm so the
  // feed reads consistently.
  if (job.sponsor) return buildSponsorLines(job);
  ...
  // Blank line then URL on its own line, followed by trailing text — the
  // trailing text is what stops FB from stripping the URL out of the post
  // (it strips when the URL is the last thing before the preview card).
  lines.push('');
  lines.push('📤 Submit your CV:');
  lines.push(uploadUrl);
  lines.push('👆 اضغط الرابط أو انسخه وشاركه');
  return { lines, uploadUrl };
}

function buildSponsorLines(job) {
  // Rezolyzer routes through wzyfa's form; the others link out directly.
  const applyUrl = job.applyUrl || `${PUBLIC_BASE}/careers?job=${job.id}`;
  const lines = [];
  // Announcement banner instead of a bare title — matches the tone the
  // page owner wanted ("Wasla is announcing a new role") and makes it
  // clear the post is coming from the brand, not a random listing.
  if (job.company) lines.push(`📢 ${job.company} تعلن عن وظيفة جديدة`);
  lines.push('');
  if (job.title) lines.push(`🎯 ${job.title}`);
  if (job.website) lines.push(`🌐 ${job.website}`);
  lines.push('');
  lines.push('📤 قدّم الآن:');
  lines.push(applyUrl);
  lines.push('👆 اضغط الرابط أو انسخه وشاركه');
  return { lines, uploadUrl: applyUrl };
}
